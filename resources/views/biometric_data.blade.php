	...
	<div class="py-12">
		<div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
			<div class="bg-white shadow-sm sm:rounded-lg p-8 border border-gray-200">
				...
				<p class="text-sm text-gray-600 mt-1">Select a .csv file exported from your biometric system. Large files (8k–10k rows) may take a while to process.</p>
				<form id="biometricForm" enctype="multipart/form-data" class="mt-6">
					...
					<div class="flex items-center gap-3">
						<button id="submitBtn" type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-5 rounded shadow-sm">
							Import
						</button>
						<span id="helperText" class="text-xs text-gray-500">Do not close the page while importing.</span>
					</div>
				</form>
			...
			<div class="bg-white shadow-sm sm:rounded-lg p-8 border border-gray-200 mt-6">
				...
				<div class="mt-6">
					<button id="exportBtn" type="button" class="bg-green-600 hover:bg-green-700 text-white font-semibold py-2 px-5 rounded shadow-sm">
						Export Database to CSV
					</button>
					<span id="exportHelperText" class="ml-3 text-xs text-gray-500">This will overwrite the existing CSV file.</span>
				</div>
			...
			$.ajax({
				...
				success: function(data) {
					if (data.success) {
						...
						exportHelperText.textContent = `Successfully exported ${data.count} employees to CSV.`;
					} else {
						exportMsg.innerHTML = data.message;
